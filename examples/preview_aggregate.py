...
def make_preview(n=131):
    ...
    for i,site in enumerate(sites):
        ...
        daily_level=np.clip(level+rng.normal(0,.17,(30,1)),.04,.92)
        amplitude=rng.uniform(.02,.25)
        power=np.clip(daily_level+amplitude*np.sin(2*np.pi*hour/24+rng.uniform(0,2*np.pi,(30,1)))
                      +rng.normal(0,.015 if site=="offshore" else .035,(30,24)),0,1)
