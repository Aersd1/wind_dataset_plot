...
| 图 | 展示形式 | 计算基础 |
|---|---|---|
| a | 海陆分组的特征分布图 | 四项运行特征跨风场 z-score；小提琴显示分布，点/粗线显示组内中位数/IQR |
| b | 海陆分组的出力分位数带 | 先逐风场计算 P10/P25/P50/P75/P90；每个分位点再取组内风场中位数及 25–75% 范围 |
| c | 时间覆盖色带 | 每个 UTC 小时有完整数据的原始数据集数；没有数据的时间为 0 |
| d | 日/周周期强度二维计数图 | 横轴日强度，纵轴周强度；每个有效风场计入一个六边形，颜色表示风场数量 |
| e | 日均出力 × 日内变化幅度的密度图 | 完整日 CF 均值与相邻小时绝对变化均值，颜色为日样本数量 |
| f | 单张日模式热图，旁列样本占比 | 24 维 CF 日曲线聚类、各组逐小时中位数；不再输出 16 张小曲线 |

默认生成六个面板和一张固定版式组合图，配色只选 `viridis` 或 `plasma`，海陆类型另用形状/线型辅助区分。**图中没有风场名称、ID 或逐风场行列；131 个风场也不分页、不省略组合图。** a 仅保留四项特征，b 仅保留两个海陆分组，d 聚合为二维计数，f 行数随实际日模式数量变化。旧配置的 `rows_per_page` 仅为兼容保留，不再影响绘图。

场站身份只从展示中移除，计算仍保留 `name`/`source`，防止把片段误计为新风场；审计 CSV 保留可追溯性。a/b/d 每个场站权重相同，e/f 每个已选日样本权重相同。b 的阴影表示**风场之间的差异**，不是置信区间，也不是混合所有小时样本后的分布。

a 中小提琴用 Gaussian KDE（Scott 带宽），各小提琴按相同最大宽度绘制，宽度不表示组内风场总数；图例标注海陆组总数。缺失特征逐列剔除，常量/单值组只画中位数及 IQR。d 只使用日/周强度同时有效的风场，并标出剔除数量；不会把缺失强度填零。

可先生成 **131 个模拟风场的版式示例**（不会读取真实数据，所有预览图均标注 SYNTHETIC STYLE PREVIEW）：

```bash
python examples/preview_aggregate.py --output results/style_preview --farms 131 --palette viridis
```

该脚本中的季节强度等为版式用模拟数值，不能作为论文结论；生产入口仍从真实 CSV 计算统计量。预览同时生成 PNG/PDF/SVG。服务器更新后建议使用新的输出目录（配置模板已改为 `results/aligned_farms_aggregate`），避免旧版逐风场图与新版文件混在一起。
...
- e/f 每个已选择日样本权重相同，因此长期数据集可能贡献更多日样本。a/b/d 则先逐场站计算，再以每个场站等权汇总。清单记录抽样规则与实际数量。
...
```text
results/
  figures/                    # 六个聚合面板及固定版式组合图：PNG / PDF / SVG
  tables/
