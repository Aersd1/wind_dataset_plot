...
**当前默认：四块主图 + 两张补充图。** a 为海陆出力分布曲线（无条形），b 为匿名风场散点，c 为月均可用风场数折线，d 为日内变化累计分布曲线，直接标注 50% 和 90% 日样本的变化阈值。详见 [逐图解释、统计口径与英文图注](docs/clear_figure.md)。
...
| 图 | 形式 | 表达的内容 |
|---|---|---|
| a 主图 | 两条出力分布曲线 | 每个 10 个百分点出力区间的时间占比；海上/陆上直接对比 |
| b 主图 | 匿名风场散点 | 平均出力（容量百分比）与平均小时变化（百分点），一点一风场 |
| c 主图 | 时间覆盖折线 | 各月平均每小时具有完整记录的不同风场数 |
| d 主图 | 日内变化累计曲线 | 日平均小时变化不超过横轴阈值的日样本占比，标出 50% / 90% 阈值 |
| e 补充 S1 | 两个周期强度箱线图 | 配对有效风场的 24 h / 168 h STL 强度分布 |
| f 补充 S2 | 完整日模式热图 | 全部聚类中位数及其日样本占比 |

默认 `plots.layout: "clear"`，生成四张主图单面板、两张补充图和一张四面板组合图。组合图为 **183 × 145 mm**，普通文字 5.5–7 pt、面板字母 8 pt，PDF 保留可编辑文字。图内无风场名和逐场站长列表；`rows_per_page` 已不影响绘图。

a 先计算每个风场的时间频率，再按风场等权平均；每组所有区间合计 100%。曲线连接区间中心，不进行额外平滑，浅色填充不是置信区间。低于 0% 和高于 100% 的值另列尾部空心点，不裁剪。b 每个风场一点；d/f 按已选完整日等权，长记录风场可能贡献更多天。c 对全局观察时段内的小时计数求月均值，包含零覆盖小时，首尾月份不补齐到整月；同时导出原小时计数和月均值。d 为精确经验累计分布，不分箱，不平滑，异常大的变化仍保留。

```bash
python examples/preview_aggregate.py --output results/clear_preview --farms 131 --palette viridis
```

以上仅生成模拟版式，全部图标注 SYNTHETIC STYLE PREVIEW，不读取真实 CSV，也不能用于论文结论。真实运行请使用新输出目录（模板为 `results/aligned_farms_clear`）。新版需要新增的频率计数和平均小时变化，服务器须重新计算；不能从旧 PNG 或五个分位数还原新图。

仅主图设置 `"plots": {"layout": "clear", "panels": ["a","b","c","d"]}`，跳过 STL 和 KMeans，仍计算完整日统计。复现前版设 `"layout": "legacy"`，其面板字母含义不同，见 [旧版说明](docs/figure_interpretation_zh.md)。
...
- d/f 每个已选择日样本权重相同，因此长期数据集可能贡献更多日样本。a/b/e 按风场计算，清单记录抽样规则与实际数量。
...
```text
results/
  figures/                    # main_a~d、supplement_e/f、figure5_clear：PNG / PDF / SVG
  tables/
    dataset_summary.csv       # 每个原始数据集一行，含特征、分位数、STL 使用量
    output_distribution.csv   # 海陆组等风场权重的出力频率，含两端越界区间
...
测试包括原始数据集归并、数字后缀保护、原文件与片段混入防护、元数据冲突、单位换算、重复和冲突时间点、不跨切割边界、缺失小时/完整日、DST、覆盖去重、周期强度、动态聚类、viridis/plasma 及六面板 PNG/PDF/SVG 完整合成测试。新增验证 131 场站仍生成七张图、SVG 不包含风场身份、分位数带按风场等权、周期密度总数等于有效风场数。不会访问用户真实 CSV 数据。
