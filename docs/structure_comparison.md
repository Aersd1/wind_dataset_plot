...
这一张独立图沿用原文 b 图“小提琴形状＋风场散点”的视觉思路，扩展为三个并排面板。它与四块总览主图一同输出，文件为 `figures/farm_structure_comparison.png/pdf/svg`。图中不再放标题、面板字母、`n=` 或最下面的 `Each point...` 说明，保留纵轴定义与海陆分组标签；以下 a/b/c 指从左到右的三个面板。配置 `plots.structure_comparison` 默认为 true；设为 false 可关闭。此选项仅对 clear 布局生效。
...
- 每组黑色短横线是该组风场指标的中位数。
- 半小提琴在某个高度越宽，表示风场指标在这个取值附近更集中。两组最大宽度一致，所以宽度不能用于比较风场总数；有效数量保存在 `structure_summary.csv`。
...
所有指标使用完整小时功率、按容量分母归一化后计算。功率原单位 MW，三个轴统一显示为风场容量的百分比。出力范围和变化在汇总表中仍可标为归一化出力的百分点，两者数值相同。变化计算不跨缺口、文件边界。每个指标分别排除缺失值，有效数量仍写入汇总表，不在图中标注；不会将缺失补零。超出常规范围的数值保留。原容量表的观测最大值替代容量事项仍适用。
